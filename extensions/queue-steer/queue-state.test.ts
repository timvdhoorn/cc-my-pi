...
function createHarness(options: { cwd?: string; projectTrusted?: boolean } = {}) {
	...
	registerBundledQueueSteer(pi as any, () => true);
...
test("bundled draft guard: busy + typed draft: escape does not pause/abort", async () => {
	const harness = createHarness();
	await harness.emit("session_start");
	await enqueue(harness, "steer", "queued steer message");
	harness.setIdle(false);
	harness.editor.setText("half-typed draft");

	harness.editor.handleInput("escape");
	// A typed draft owns Esc (double-esc-clear); the queue must not react.
	assert.equal(harness.aborted, false);
	assert.equal(harness.editor.getText(), "half-typed draft");
});

test("bundled draft guard: busy + empty chatbox: escape pauses and aborts", async () => {
	const harness = createHarness();
	await harness.emit("session_start");
	await enqueue(harness, "steer", "queued steer message");
	harness.setIdle(false);
