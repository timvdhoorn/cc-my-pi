...
function makeController() {
	...
	const controller: CcToolsSettingsController = {
		...
		apply(id, value) {
			...
			(snap as any)[key] =
				typeof snap[key] === "boolean" ? value === "on" : value;
		},
	};
...
test("wizard: intro Esc (raw and kitty CSI-u) resolves skip-once", async () => {
	for (const esc of [ESC, "\x1b[27u"]) {
		...
		assert.equal(
			await done,
			"skip-once",
			`esc encoding ${JSON.stringify(esc)}`,
		);
	}
});
...
test("wizard: includes 1.3 tool-group compatibility settings", () => {
	...
	assert.equal(
		rows.get("groupToolCallsAcrossTurns")?.current(baseSnapshot),
		"on",
	);
	assert.equal(
		rows.get("showCompactToolStatusDots")?.current(baseSnapshot),
		"off",
	);
});
...
test("wizard: custom steps derive from SETTING_ORDER and enter through all closes it", async () => {
	...
	assert.equal(
		total,
		CORE_STEPS.length,
		"custom mode covers every core setting row",
	);
...
test("wizard: every frame renders to a constant height", async () => {
	...
	assert.equal(
		companionsHeight,
		introHeight,
		"companions screen matches intro height",
	);
	...
	assert.equal(
		nextHeight,
		introHeight,
		"a different preview shape keeps the height",
	);
});
...
test("wizard: a custom out-of-list value is the selected default and cycles into the list", async () => {
	...
	assert.ok(
		atStep.includes(`● ${custom}`),
		"custom value is the selected default",
	);
	assert.equal(
		applyCalls.length,
		0,
		"entering steps without cycling changes nothing",
	);
...
test("wizard: standard mode installs selected companions in list order then completes", async () => {
	...
	const second = COMPANION_PACKAGES[1]!;
	const companionApplies = applyCalls.filter(([id]) =>
		id.startsWith("companion:"),
	);
	assert.deepEqual(companionApplies, [
		[`companion:${second.source}`, COMPANION_INSTALL_VALUE],
	]);
	assert.equal(await done, "completed");
});
...
test("wizard: custom mode after companions reaches the first core step", async () => {
	...
	assert.equal(
		stepCount(component),
		CORE_STEPS.length,
		"custom steps are the core settings",
	);
	assert.ok(
...
test("wizard: installed companion renders ✓ installed and space does not install", async () => {
	...
	component.handleInput("\r"); // -> companions
	const installedName = COMPANION_PACKAGES.find(
		(c) => c.source === "npm:pi-context-view",
	)!.name;
	assert.ok(
	...
	assert.equal(
		applyCalls.filter(([id]) => id.startsWith("companion:")).length,
		0,
		"no install",
	);
});
...
test("wizard: 'b' on companions returns to the intro", async () => {
	...
	assert.ok(
		component.render(80).join("\n").includes("enter start"),
		"intro legend shown again",
	);
});
...
test("companions: one row per companion, ids companion:<source>, after regular rows", () => {
	const companionRows = SETTING_ORDER.filter((def) =>
		String(def.id).startsWith("companion:"),
	);
	...
	const firstCompanion = SETTING_ORDER.findIndex((def) =>
		String(def.id).startsWith("companion:"),
	);
	assert.equal(
		firstCompanion,
		SETTING_ORDER.length - COMPANION_PACKAGES.length,
	);
...
test("companions: installed row shows ✓ installed, not-installed shows ✗ not installed", () => {
	...
	const installed = SETTING_ORDER.find(
		(def) => def.id === "companion:npm:pi-context-view",
	)!;
	const notInstalled = SETTING_ORDER.find(
		(def) => def.id === "companion:npm:pi-subagents",
	)!;
...
test("companions: wizardStepValues offers the install action; installed row keeps ✓ as current", () => {
	...
	assert.ok(
		notInstalledValues.includes(COMPANION_INSTALL_VALUE),
		"install action offered",
	);

	const installedValues = wizardStepValues(installed, snap);
	assert.ok(
		installedValues.includes(COMPANION_INSTALL_VALUE),
		"install action still offered",
	);
	assert.equal(
		installedValues[0],
		"✓ installed",
		"current value is the selected default",
	);
});
...
test("companions: the companions checkbox screen keeps the constant height", async () => {
	...
	assert.equal(
		component.render(80).length,
		introHeight,
		"companions screen keeps the height",
	);
